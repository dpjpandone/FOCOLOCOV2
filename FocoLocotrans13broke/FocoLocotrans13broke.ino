...
hardstopdisp = map (hardstop,astop,bstop,0,999);
    ...
    hardstop = map(hardstop,0,1023,macro,infinity); 
...
astopdisp = map (astop,0,1023,0,999);
  ...
  m=sprintf (buffer, "%03d POS A:%03d",macrodisp, astopdisp);
  lcd.setCursor(1, 1);
  lcd.write(byte(5));
  lcd.print(buffer); ///< You will get “000 00 00 00 00″


  n=sprintf (buffer, "%03d %03d B:%03d",infinitydisp,hardstopdisp,bstopdisp);
  lcd.setCursor(2, 1);
  lcd.write(byte(4));
  lcd.print(buffer); ///< You will get “000 00 00 00 00″
