...
// Function to change the Slave Select pin
// default is D10, but can change here.  
//	NOTE: Must call this BEFORE radio.initialize
void RFM69::setCS(byte newSPISlaveSelect) {
  _slaveSelectPin = newSPISlaveSelect;
  pinMode(_slaveSelectPin, OUTPUT);
}

// Anarduino Added, 2013-11-13  
//	NOTE: It should be run immediately AFTER the radio.initialize function
void RFM69::setIRQ(byte newInterruptPin) {  // valid as 2 or 3 (i.e. D2/D3)
  if(newInterruptPin==2) {
	  if(_interruptPin==3) // if previously defined/set as 3, then detach it
			detachInterrupt(1);
	  _interruptPin = newInterruptPin;
	  attachInterrupt(0, RFM69::isr0, RISING);
  }else if(newInterruptPin==3) {
	  if(_interruptPin==2) 		// if previously defined/set as 2, then detach it
			detachInterrupt(0);
	  _interruptPin = newInterruptPin;
