...
static uint8_t rf12_byte (uint8_t out) {
	...
	// RNL - added timeout, so as not to hang here. 
	// TODO: determine why it hangs and more elegant way to resolve, or notify of issue
	long l=millis();
    while (!(SPSR & _BV(SPIF))) {
		if((millis()-l)>200)
			break;
...
uint8_t rf12_initialize (uint8_t id, uint8_t band, uint8_t g) {
    ...
    rf12_xfer(0x80F7); // RNL 12/2, 915Mhz 12.0pF
    rf12_xfer(0xA680); // TODO: Need to get proper setting here for Frequency Setting Command ???
